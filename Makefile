...
# All Rights Reserved
#
# Modificated from stock 17.1 Makefile
# by Svein Seldal <sveinse@seldal.com>
# Mod v1, 2017-11-19
#
# Local edits are labeled with "sveinse:"
#
################################################
...
PRELOADER_DIR := $(OUTPUT_DIR)/preloader
...
QUARTUS_SOF := $(OUTPUT_DIR)/$(QUARTUS_BASE).sof
...
ifeq ($(HAVE_QUARTUS),1)
SCRUB_CLEAN_FILES += $(QUARTUS_SOF) $(QUARTUS_RBF) $(OUTPUT_DIR) hps_isw_handoff
endif
...
# sveinse: Add support for FAT
PRELOADER_FAT_SUPPORT ?= 1
ifeq ($(PRELOADER_FAT_SUPPORT),1)
PRELOADER_EXTRA_ARGS += --set spl.boot.FAT_SUPPORT true
endif

PRELOADER_EXTRA_ARGS += --set spl.boot.FAT_LOAD_PAYLOAD_NAME u-boot.img

.PHONY: preloader
preloader: $(PRELOADER_STAMP)

# sveinse: Add clean for preloader
.PHONY: preloader-clean
preloader-clean:
	$(RM) -rf $(PRELOADER_DIR) $(PRELOADER_STAMP) $(UBOOT_STAMP)
...
# sveinse: Use our board info files checked into git
DTS.BOARDINFO ?= board_info/$(QSYS_BASE)_board_info.xml
DTS.COMMON ?= board_info/hps_common_board_info.xml
...
DEVICE_TREE_SOURCE := $(patsubst %.sopcinfo,$(OUTPUT_DIR)/%.dts,$(QSYS_SOPCINFO))
...
$(DEVICE_TREE_SOURCE): $(OUTPUT_DIR)/%.dts: %.sopcinfo $(DTS.EXTRA_DEPS)
	$(call dts.sopc2dts,$<,$@)


# Device Tree Blob (dtb)
DEVICE_TREE_BLOB := $(patsubst %.sopcinfo,$(OUTPUT_DIR)/%.dtb,$(QSYS_SOPCINFO))
...
$(DEVICE_TREE_BLOB): $(OUTPUT_DIR)/%.dtb: $(OUTPUT_DIR)/%.dts
...
# sveinse: Disable, the project provides its own
disable.boot.script: Makefile
	@$(RM) $@
	@$(ECHO) "Generating $@"
...
################################################
# sveinse: Generate boot script from sources
.PHONY: u-boot.scr
u-boot.scr: $(OUTPUT_BOOT)/u-boot.scr

# sveinse: Generate the bootscript 
$(OUTPUT_BOOT)/boot.script: boot/boot.script
	@$(MKDIR) $(@D)
	$(DOS2UNIX) -n $< $@
$(OUTPUT_BOOT)/u-boot.scr: $(OUTPUT_BOOT)/boot.script $(UBOOT_MKIMAGE)
	$(UBOOT_MKIMAGE) -A arm -O linux -T script -C none -a 0 -e 0 -n "bootscript" -d $< $@

# sveinse: Want everything in the output dir
SD_FAT_TGZ ?= $(OUTPUT_DIR)/sd_fat.tar.gz

# sveinse: Copy everything to $(OUTPUT_BOOT) to collect the boot files
SD_FAT_TGZ_DEPS += \
	$(OUTPUT_BOOT)/boot.script \
	$(OUTPUT_BOOT)/u-boot.scr \
	$(OUTPUT_BOOT)/$(notdir $(PRELOADER_BIN)) \
	$(OUTPUT_BOOT)/$(notdir $(NEXTSTAGE_BIN)) \
	$(OUTPUT_BOOT)/$(notdir $(QUARTUS_RBF)) \
	$(OUTPUT_BOOT)/$(notdir $(QUARTUS_SOF)) \
	$(OUTPUT_BOOT)/$(notdir $(DEVICE_TREE_SOURCE)) \
	$(OUTPUT_BOOT)/$(notdir $(DEVICE_TREE_BLOB)) \

# sveinse: Collect files from project into 
$(OUTPUT_BOOT)/$(notdir $(PRELOADER_BIN)): $(PRELOADER_STAMP)
	cp -f $(PRELOADER_BIN) $@
$(OUTPUT_BOOT)/$(notdir $(NEXTSTAGE_BIN)): $(UBOOT_STAMP)
	cp -f $(NEXTSTAGE_BIN) $@
$(OUTPUT_BOOT)/$(notdir $(QUARTUS_RBF)): $(QUARTUS_RBF)
	cp -f $^ $@
$(OUTPUT_BOOT)/$(notdir $(QUARTUS_SOF)): $(QUARTUS_SOF)
	cp -f $^ $@
$(OUTPUT_BOOT)/$(notdir $(DEVICE_TREE_BLOB)): $(DEVICE_TREE_BLOB)
	cp -f $^ $@
$(OUTPUT_BOOT)/$(notdir $(DEVICE_TREE_SOURCE)): $(DEVICE_TREE_SOURCE)
	cp -f $^ $@

.PHONY: boot
boot: $(SD_FAT_TGZ_DEPS)

$(SD_FAT_TGZ): $(SD_FAT_TGZ_DEPS)
	@$(RM) $@
	@$(MKDIR) $(@D)
	$(TAR) -C $(OUTPUT_DIR)/boot -czf $@ .

.PHONY: sd-fat
sd-fat: $(SD_FAT_TGZ)

HELP_TARGETS += sd-fat
sd-fat.HELP := Make a tgz ($(SD_FAT_TGZ)) containing the boot partition
...
################################################
# sveinse: Add fpga tgz for output files

FPGA_TGZ ?= $(OUTPUT_DIR)/fpga_files.tar.gz

FPGA_TGZ_DEPS += \
	$(QUARTUS_STAMP) \

.PHONY: fpga fpga-tgz fpga-unpack
fpga fpga-tgz: $(FPGA_TGZ)

$(FPGA_TGZ): $(FPGA_TGZ_DEPS)
	@$(RM) $@
	@$(MKDIR) $(@D)
	$(TAR) -czf $@ \
		$(wildcard $(QUARTUS_SOF)) \
		$(wildcard *.sopcinfo) \
		$(wildcard hps_isw_handoff/*)

HELP_TARGETS += fpga-tgz
fpga-tgz.HELP := Make a tgz ($(FPGA_TGZ)) with the FPGA project output for later use

fpga-unpack:
	tar -xvf $(FPGA_TGZ)
	$(MAKE) quartus_ok

HELP_TARGETS += fpga-unpack
fpga-unpack.HELP := Unpack existing FPGA_TGZ= to allow building boot files without compiling FPGA project

# sveinse: Allow make quartus_ok to touch any stamps generated by manual
# Quartus use to prevent Quartus/Qsys rebuilds when building preloader and
# bootloader from a pre-built image.
.PHONY: quartus_ok
quartus_ok:
	if [ -e $(QUARTUS_SOF) ]; then \
		$(MKDIR) -p $(dir $(QUARTUS_STAMP)); \
		$(TOUCH) $(QSYS_STAMP) $(QUARTUS_STAMP) $(QUARTUS_PIN_ASSIGNMENTS_STAMP); \
	fi
...
tgz_scrub_clean:
	...
	$(FIND) $(PRELOADER_DIR) \( -name '*.o' -o -name '.depend*' -o -name '*.d' -o -name '*.dep' \) -delete || true
